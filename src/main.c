...
int header_b_y_offset = 0; //vertical offset of the right side of the header
int header_time_width = 0; //width of the time layer
int header_time_height = 0; //height of the time layer
int header_b_height = 0; //height of the weekday layer
int screen_width = 144; //width of the pebble screen
int header_date_height = 0; //height of the date layer
...
int create_day_separator_layer(int i, int y, Layer* parent, caltime_t day) {
	...
	text_layer_set_text_alignment(day_separator_layers[i], GTextAlignmentCenter);
...
static void handle_bluetooth(bool connected) {
	text_layer_set_text(text_layer_bluetooth, connected ? "Conn" : "Discon");
}
...
void create_header(Layer *window_layer) {
	if (!(settings_get_bool_flags() & SETTINGS_BOOL_SHOW_CLOCK_HEADER)) { //stop creating here if user settings permit
		...
		//Create time layer
		text_layer_time = text_layer_create(GRect(0, header_time_y_offset, header_time_width, header_time_height));
		...
		//Create date layer
		text_layer_date = text_layer_create(GRect(0, header_time_y_offset+header_time_height, screen_width, header_date_height));
		...
		text_layer_battery = text_layer_create(GRect(header_time_width, header_b_y_offset, 144-header_time_width, header_b_height));
		text_layer_set_background_color(text_layer_battery, GColorBlack);
		...
		text_layer_set_overflow_mode(text_layer_battery, GTextOverflowModeWordWrap);
		text_layer_set_font(text_layer_battery, date_font);
		layer_add_child(window_layer, text_layer_get_layer(text_layer_battery));

		//Create bluetooth layer
		text_layer_bluetooth = text_layer_create(GRect(header_time_width, header_b_y_offset+header_b_height, 144-header_time_width, header_b_height));
		text_layer_set_background_color(text_layer_bluetooth, GColorBlack);
		text_layer_set_text_color(text_layer_bluetooth, GColorWhite);
		text_layer_set_text_alignment(text_layer_bluetooth, GTextAlignmentCenter);
		text_layer_set_overflow_mode(text_layer_bluetooth, GTextOverflowModeWordWrap);
		text_layer_set_font(text_layer_bluetooth, date_font);
		layer_add_child(window_layer, text_layer_get_layer(text_layer_bluetooth));
...
void destroy_header() {
	...
	if (text_layer_battery != 0) text_layer_destroy(text_layer_battery);
	if (text_layer_bluetooth != 0) text_layer_destroy(text_layer_bluetooth);
	if (sync_indicator_layer != 0) text_layer_destroy(sync_indicator_layer);

	text_layer_time = 0;
	text_layer_date = 0;
	text_layer_battery = 0;
	text_layer_bluetooth = 0;
